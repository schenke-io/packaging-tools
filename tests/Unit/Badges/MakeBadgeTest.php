...
it('can define a badge', function () {
    $badge = MakeBadge::define('Subject', 'Status', 'Color');
...
})->with(['Coverage', 'PhpStan', 'Infection', 'PHP'])->throws(PackagingToolException::class);
...
it('auto outputs message when badge is generated by mocking Config::output', function () {
    ...
    $mock->shouldReceive('output')
        ->atLeast()->once()
        ->with(SetupMessages::infoBadgeGenerated, Mockery::any());

    Config::$outputHandler = fn (...$args) => $mock->output(...$args);
